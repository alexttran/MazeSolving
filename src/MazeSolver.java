...
public class MazeSolver {
    ...
    private static int numMoves = 0; // Allows us to keep track of how many
    // recursive calls to findPath are required to solve the maze.
    private String[][] maze;
    int numRows;
    int numCols;
    ...
    private boolean findPath(int col, int row) {
        numMoves++;
        //System.out.println(this);
        if (col < 0 || row < 0 || col > maze[0].length - 1 || row > maze.length - 1) {
            return false;
        }
        ...
        if (maze[row][col].equals(".")) {
            ...
            if (findPath(col + 1, row))
                return true;
            else if (findPath(col - 1, row))
                return true;
            else if (findPath(col, row + 1))
                return true;
            else if (findPath(col, row - 1))
                return true;
        } else
